...
class String {
    ...
    public static function implode($separator,$items,$except=false){
        $res = '';
        $counter = 0;
        if ($except){
            foreach ($items as $key=>$value){
                if (!in_array($key,$except)){
                    if ($counter) $res .= $separator.$value;
                    else $res .= $value;
                    $counter ++;
                }
            }
    ...
    public static function trim($string,$charlist=" \t\n\r\0\x0B"){
        if (empty($charlist)) {
            ...
            return preg_replace ("/(^[".preg_quote($charlist,'/')."]+)|([".preg_quote($charlist)."]+$)/us", '', $string);
        }
    }
    ...
    /**
     * search aliases (eg {alias})  in string and replace them by paths from Yii::getPathOfAlias
     * @param string $content
     * @return Ambigous <mixed, boolean, string, multitype:string >|unknown|mixed
     */
    public static function convertAliases($content)
    {
        $content = preg_replace_callback('/{.*}/misx', function($matches){
            if (($alias = Yii::getPathOfAlias(trim($matches[0],'{}')))) {
        	   return $alias;
            } else {
               return $matches[0];
            }
